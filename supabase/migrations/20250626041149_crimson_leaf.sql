...
-- Create storage buckets for user content if they don't exist
DO $$
BEGIN
  -- Create user-clips bucket
  IF NOT EXISTS (
    SELECT 1 FROM storage.buckets WHERE id = 'user-clips'
  ) THEN
    INSERT INTO storage.buckets (id, name, public)
    VALUES ('user-clips', 'user-clips', false);
  END IF;
  
  -- Create user-music bucket
  IF NOT EXISTS (
    SELECT 1 FROM storage.buckets WHERE id = 'user-music'
  ) THEN
    INSERT INTO storage.buckets (id, name, public)
    VALUES ('user-music', 'user-music', false);
  END IF;
  
  -- Create clip-thumbnails bucket
  IF NOT EXISTS (
    SELECT 1 FROM storage.buckets WHERE id = 'clip-thumbnails'
  ) THEN
    INSERT INTO storage.buckets (id, name, public)
    VALUES ('clip-thumbnails', 'clip-thumbnails', true);
  END IF;
  
  -- Create scene-videos bucket
  IF NOT EXISTS (
    SELECT 1 FROM storage.buckets WHERE id = 'scene-videos'
  ) THEN
    INSERT INTO storage.buckets (id, name, public)
    VALUES ('scene-videos', 'scene-videos', false);
  END IF;
  
  -- Create temp-images-for-video-generation bucket
  IF NOT EXISTS (
    SELECT 1 FROM storage.buckets WHERE id = 'temp-images-for-video-generation'
  ) THEN
    INSERT INTO storage.buckets (id, name, public)
    VALUES ('temp-images-for-video-generation', 'temp-images-for-video-generation', true);
  END IF;
END $$;
...
DO $$ BEGIN
  ...
  DROP POLICY IF EXISTS "Users can view their own clips" ON storage.objects;
  ...
  DROP POLICY IF EXISTS "Users can view their own music" ON storage.objects;
  DROP POLICY IF EXISTS "Users can delete their own music" ON storage.objects;
  
  -- Clip thumbnails policies
  DROP POLICY IF EXISTS "Users can upload clip thumbnails" ON storage.objects;
  DROP POLICY IF EXISTS "Clip thumbnails are publicly accessible" ON storage.objects;
  
  -- Scene videos policies
  DROP POLICY IF EXISTS "Users can upload scene videos" ON storage.objects;
  DROP POLICY IF EXISTS "Users can view their own scene videos" ON storage.objects;
  
  -- Temp images policies
  DROP POLICY IF EXISTS "Users can upload temp images" ON storage.objects;
  DROP POLICY IF EXISTS "Temp images are publicly accessible" ON storage.objects;
END $$;

-- Storage policies for user-clips bucket
CREATE POLICY "Users can upload their own clips"
ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (
  bucket_id = 'user-clips' AND
...
-- Storage policies for clip-thumbnails bucket
CREATE POLICY "Users can upload clip thumbnails"
ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (
...
CREATE POLICY "Clip thumbnails are publicly accessible"
ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'clip-thumbnails');

-- Storage policies for scene-videos bucket
...
CREATE POLICY "Users can view their own scene videos"
ON storage.objects FOR SELECT
TO authenticated
USING (
  bucket_id = 'scene-videos' AND
...
-- Storage policies for temp-images-for-video-generation bucket
CREATE POLICY "Users can upload temp images"
ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (
  bucket_id = 'temp-images-for-video-generation' AND
  auth.uid()::text = (storage.foldername(name))[1]
);
...
ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'temp-images-for-video-generation');